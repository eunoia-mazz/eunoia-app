...
export function Therapist() {
  ...
  const handleViewPatients = (id) => {
    fetch(`http://localhost:5000/view_patients/${id}`)
      .then(res => res.json())
      .then(data => {
  ...
  return (
    <>
      ...
      <DashboardShell>
        <DashboardHeader heading="Therapists" text="Manage and monitor therapist accounts.">
          <Button onClick={() => { setFormData({ id: null, name: "", designation: "", qualification: "", location: "", is_available: true,}); setShowForm(true);}} className="bg-blue-500 text-white hover:bg-blue-600">
            Add Therapist
          </Button>
        </DashboardHeader>
        <Card>
                ...
                <TableBody>
                  {therapists.map((t) => (
                    <TableRow key={t.id}>
                      ...
                      <TableCell>{t.patients_treated + t.patients_queue}</TableCell>
                      <TableCell>
                        <Button size="sm" onClick={() => handleEdit(t)} className="mr-2 bg-blue-500 text-white">
                          Edit
                        </Button>
                        <Button size="sm" onClick={() => handleViewPatients(t.id)} className="bg-blue-500 text-white">
                          View Patients
                        </Button>
                      </TableCell>
                    </TableRow>
        ...
        {showForm && (
          <Card className="mt-4 p-4 bg-gray-100">
            <CardTitle>{formData.id ? "Edit Therapist" : "Add Therapist"}</CardTitle>
            <CardContent className="flex flex-col gap-3 mt-2">
              ...
              <input name="qualification" placeholder="Qualification" value={formData.qualification} onChange={handleInputChange} />
        ...
        {patientsList && (
          <Card className="mt-4 p-4 bg-yellow-50">
            ...
            <CardContent>
              ...
              <Button className="mt-3" onClick={() => setPatientsList(null)}>Close</Button>
            </CardContent>
          </Card>
        )}
      </DashboardShell>
