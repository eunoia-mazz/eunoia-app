  ...
  return (
    <div className="p-5 bg-blue-50 min-h-screen">
        ...
        <div className="grid grid-cols-4 gap-4 justify-center">
          {moods.map((mood, index) => (
            <div key={index} className="flex flex-col items-center">
              <button
                className={`text-4xl ${
                  selectedMood === mood.emoji ? "bg-blue-200" : "bg-transparent"
                } p-2 rounded-md`}
                onClick={() => setSelectedMood(mood.emoji)}
              >
                {mood.emoji}
              </button>
              <span className="text-sm text-gray-600 mt-2">{mood.label}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <footer className="text-center mt-10">
        <p className="text-xl font-semibold text-gray-600 italic">{quote}</p>
      </footer>
    </div>
  );
}
